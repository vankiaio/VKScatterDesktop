...
const saveFile = (filepath) => {
    return new Promise(resolve => {
        ...
        const name = `${filepath}/ttmc__${store.state.scatter.hash.substr(0,4)}-${store.state.scatter.hash.slice(-4)}__${store.state.scatter.meta.version}__${month}-${year}.json`;
        try {
