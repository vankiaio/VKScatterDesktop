...
export default class LanguageService {
	...
	static getLanguageNames(){
		return fetch(`http://ttmc_api.vankia.net:3030/vktapi/v1/languages?names=1`).then(r => r.json())
			.catch(err => {
			return ["English"];
		})
	}
	...
	static getLanguage(name){
		return fetch(`http://ttmc_api.vankia.net:3030/vktapi/v1/languages?name=${name}`).then(r => r.json())
		.catch(err => {
			return null;
		})
	}
